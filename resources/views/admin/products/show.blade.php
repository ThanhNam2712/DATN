                            ...
                            <h6 class="mb-4 text-15">Update Product : {{$product->name}}</h6>
                                ...
                                <div class="grid grid-cols-1 gap-5 lg:grid-cols-2 xl:grid-cols-12">
                                    ...
                                    <div class="xl:col-span-6">
                                        ...
                                        <input disabled type="text" value="{{$product->description}}"  id="productNameInput" name="description" class="form-input border-slate-200 dark:border-zink-500 focus:outline-none focus:border-custom-500 disabled:bg-slate-100 dark:disabled:bg-zink-600 disabled:border-slate-300 dark:disabled:border-zink-500 dark:disabled:text-zink-200 disabled:text-slate-500 dark:text-zink-100 dark:bg-zink-700 dark:focus:border-custom-800 placeholder:text-slate-400 dark:placeholder:text-zink-200" placeholder="Product title" >
                                        <p class="mt-1 text-sm text-slate-400 dark:text-zink-200">Do not exceed 20 characters when entering the product name.</p>
                                        @error('description')
                                    ...
                                    <div class="xl:col-span-4">
                                        ...
                                        <div class="relative inline-block w-10 align-middle transition duration-200 ease-in ltr:mr-2 rtl:ml-2">
                                            <input disabled type="checkbox" {{ $product->is_new ? 'checked' : ''  }} name="is_new" id="orangeIconSwitch" class="absolute block size-5 transition duration-300 ease-linear border-2 border-slate-200 dark:border-zink-500 rounded-full appearance-none cursor-pointer bg-white/80 dark:bg-zink-600 peer/published checked:bg-white dark:checked:bg-white ltr:checked:right-0 rtl:checked:left-0 checked:bg-none checked:border-orange-500 dark:checked:border-orange-500 arrow-none after:absolute after:text-slate-500 dark:after:text-zink-200 after:content-['\eb99'] after:text-xs after:inset-0 after:flex after:items-center after:justify-center after:font-remix after:leading-none checked:after:text-orange-500 dark:checked:after:text-orange-500 checked:after:content-['\eb7b']" >
                                    ...
                                    <div class="xl:col-span-4">
                                        ...
                                        <div class="relative inline-block w-10 align-middle transition duration-200 ease-in ltr:mr-2 rtl:ml-2">
                                            <input disabled type="checkbox" {{ $product->is_show_home ? 'checked' : '' }}  name="is_show_home" id="is_show_home" class="absolute block size-5 transition duration-300 ease-linear border-2 border-slate-200 dark:border-zink-500 rounded-full appearance-none cursor-pointer bg-white/80 dark:bg-zink-600 peer/published checked:bg-white dark:checked:bg-white ltr:checked:right-0 rtl:checked:left-0 checked:bg-none checked:border-yellow-500 dark:checked:border-yellow-500 arrow-none after:absolute after:text-slate-500 dark:after:text-zink-200 after:content-['\eb99'] after:text-xs after:inset-0 after:flex after:items-center after:justify-center after:font-remix after:leading-none checked:after:text-yellow-500 dark:checked:after:text-yellow-500 checked:after:content-['\eb7b']" >
                                            <label for="yellowIconSwitch" class="block h-5 overflow-hidden duration-300 ease-linear border rounded-full cursor-pointer cursor-pointertransition border-slate-200 dark:border-zink-500 bg-slate-200 dark:bg-zink-600 peer-checked/published:bg-yellow-500 peer-checked/published:border-yellow-500"></label>
                                        </div>
                                    </div>
                                    <div class="xl:col-span-4">
                                                ...
                                                <div class="dropzone needsclick p-0 dz-clickable" id="dropzone-basic">
                                                    <div class="dz-message needsclick">
                                                        ...
                                                        <span class="note needsclick btn btn-sm btn-label-primary" id="btnBrowse">
                                                            Browse image
                                                            <input disabled type="file" name="image" id="fileInput">
                                                        </span>
                                                    </div>
                                                </div>
                                            </div>
                                    ...
                                    @foreach($product->variant as $index => $list)
                                        <div id="variant-{{ $index }}">
                                            <div style="margin-top: 38px" class="mt-5 grid grid-cols-1 gap-5 lg:grid-cols-2 xl:grid-cols-12">
                                                {{-- Products variants --}}
                                                @if(isset($list->id))
                                                    <input disabled type="hidden" name="variants[{{ $index }}][id]" value="{{ $list->id }}">
                                                @endif

                                                <div class="xl:col-span-4">
                                                    <label for="productPrice" class="inline-block mb-2 text-base font-medium">Price</label>
                                                    <input disabled type="number" name="variants[{{ $index ?? variants[0]}}][price]" id="productPrice" class="form-input ..." value="{{ $list->price }}" required="">
                                                    @error('price')
                                                    <div class="mb-3 px-4 py-3 text-sm bg-white border rounded-md border-custom-300 text-custom-500 dark:bg-zink-700 dark:border-custom-500">
                                                        <span class="font-bold">{{ $message }}</span>
                                                    </div>
                                                ...
                                                <div class="xl:col-span-4">
                                                    ...
                                                    <input disabled type="number" name="variants[{{ $index ?? variants[0] }}][price_sale]" id="productDiscounts" class="form-input ..." value="{{ $list->price_sale }}" required="">
                                                    @error('price_sale')
                                                    <div class="mb-3 px-4 py-3 text-sm bg-white border rounded-md border-custom-300 text-custom-500 dark:bg-zink-700 dark:border-custom-500">
                                                        <span class="font-bold">{{ $message }}</span>
                                                    </div>
                                                ...
                                                <div class="xl:col-span-4">
                                                    ...
                                                    <input disabled type="number" id="qualityInput" name="variants[{{ $index ?? variants[0] }}][quantity]" class="form-input ..." value="{{ $list->quantity }}" required="">
                                                    @error('quantity')
                                                    <div class="mb-3 px-4 py-3 text-sm bg-white border rounded-md border-custom-300 text-custom-500 dark:bg-zink-700 dark:border-custom-500">
                                                        <span class="font-bold">{{ $message }}</span>
                                                    </div>
                                                ...
                                                <div class="xl:col-span-4">
                                                    ...
                                                    <select disabled class="form-input ..." name="variants[{{ $index ?? variants[0] }}][product_color_id]" id="categorySelect">
                                                ...
                                                <div class="xl:col-span-4">
                                                    ...
                                                    <select disabled class="form-input ..." name="variants[{{ $index ?? variants[0] }}][product_size_id]" id="categorySelect">
                                ...
                                <div class="flex justify-end gap-2 mt-4">
                                    ...
                                    <a href="../admin/products" class="text-white btn bg-custom-500 border-custom-500 hover:text-white hover:bg-custom-600 hover:border-custom-600 focus:text-white focus:bg-custom-600 focus:border-custom-600 focus:ring focus:ring-custom-100 active:text-white active:bg-custom-600 active:border-custom-600 active:ring active:ring-custom-100 dark:ring-custom-400/20">Edit Product</a>
                                </div>
                        </div>
                    </div><!--end card-->
                </div><!--end col-->
