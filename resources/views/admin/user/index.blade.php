                            ...
                            <table class="w-full border-separate table-custom border-spacing-y-1 whitespace-nowrap">
                                <thead class="text-left">
                                    <tr class="relative rounded-md bg-slate-100 dark:bg-zink-600 after:absolute ltr:after:border-l-2 rtl:after:border-r-2 ltr:after:left-0 rtl:after:right-0 after:top-0 after:bottom-0 after:border-transparent [&.active]:after:border-custom-500 [&.active]:bg-slate-100 dark:[&.active]:bg-zink-600">
                                        ...
                                        <th class="px-3.5 py-2.5 first:pl-5 last:pr-5 font-semibold sort" data-sort="status">Chức vụ</th>
                                ...
                                <tbody class="list">
                                    ...
                                    <tr class="relative rounded-md after:absolute ...">
                                        <td class="px-3.5 py-2.5 first:pl-5 last:pr-5">
                                            <div class="flex items-center h-full">
                                                <input id="Checkbox{{ $user->id }}" class="size-4 bg-white border ..." type="checkbox">
                                            </div>
                                        </td>
                                        <td class="px-3.5 py-2.5 first:pl-5 last:pr-5">
                                            <a href="#!" class="transition-all duration-150 ease-linear ...">#{{ $user->id }}</a>
                                        </td>
                                        <td class="px-3.5 py-2.5 first:pl-5 last:pr-5">
                                            <div class="flex items-center gap-2">
                                                <div class="flex items-center ...">
                                                    <img src="{{ asset('assets/images/avatar-2.png') }}" alt="{{ $user->name }}" class="h-10 rounded-full">
                                                </div>
                                                <div class="grow">
                                        ...
                                        <td class="px-3.5 py-2.5 first:pl-5 last:pr-5 location">
                                            @if($user->addresses->isNotEmpty())
                                                @foreach($user->addresses as $address)
                                                    <div>
                                                        {{ $address->Apartment }}, {{ $address->Neighborhood }},  {{ $address->district }}, {{ $address->Province }}
                                                    </div>
                                                @endforeach
                                            @else
                                                Chưa có
                                        ...
                                        <td class="px-3.5 py-2.5 first:pl-5 last:pr-5 phone-number">{{ $user->sdt ?? 'N/A' }}</td>

                                        <!-- Hiển thị role của user -->
                                        <td class="px-3.5 py-2.5 first:pl-5 last:pr-5">
                                            <span class="badge badge-soft-primary">{{ $user->role->name ?? 'Chưa có' }}</span>
                                        </td>

                                        <td class="px-3.5 py-2.5 first:pl-5 last:pr-5">
                                            <div class="relative dropdown">
                                                <button class="flex items-center justify-center size-[30px] dropdown-toggle p-0 text-slate-500 btn bg-slate-100 hover:text-white hover:bg-slate-600 focus:text-white focus:bg-slate-600 focus:ring focus:ring-slate-100 active:text-white active:bg-slate-600 active:ring active:ring-slate-100 dark:bg-slate-500/20 dark:text-slate-400 dark:hover:bg-slate-500 dark:hover:text-white dark:focus:bg-slate-500 dark:focus:text-white dark:active:bg-slate-500 dark:active:text-white dark:ring-slate-400/20" id="usersAction{{ $user->id }}" data-bs-toggle="dropdown">
                                                    <i data-lucide="more-horizontal" class="size-3"></i>
                                                </button>
                                                <ul class="absolute z-50 hidden py-2 mt-1 ltr:text-left rtl:text-right list-none bg-white rounded-md shadow-md dropdown-menu min-w-[10rem] dark:bg-zink-600" aria-labelledby="usersAction{{ $user->id }}">
                                                    <li>
                                                        {{-- <form action="{{ route('user.block', $user->id) }}" method="POST">
                                                            ...
                                                            <button type="submit" class="block px-4 py-1.5 text-base transition-all duration-200 ease-linear text-slate-600 dropdown-item hover:bg-slate-100 hover:text-slate-500 focus:bg-slate-100 focus:text-slate-500 dark:text-zink-100 dark:hover:bg-zink-500 dark:hover:text-zink-200 dark:focus:bg-zink-500 dark:focus:text-zink-200">
                                                                <i data-lucide="slash" class="inline-block size-3 ltr:mr-1 rtl:ml-1"></i> {{ $user->is_blocked ? 'Hủy chặn' : 'Chặn' }}
                                                            </button>
                                                        </form> --}}
                                                    </li>
                                                    {{-- <li>
                                                        <a class="block px-4 py-1.5 text-base transition-all duration-200 ease-linear text-slate-600 dropdown-item hover:bg-slate-100 hover:text-slate-500 focus:bg-slate-100 focus:text-slate-500 dark:text-zink-100 dark:hover:bg-zink-500 dark:hover:text-zink-200 dark:focus:bg-zink-500 dark:focus:text-zink-200" href="{{ route('user.edit', $user->id) }}">
                                                            <i data-lucide="file-edit" class="inline-block size-3 ltr:mr-1 rtl:ml-1"></i> Sửa
                                                        </a>
                                                    </li> --}}
                                                    <li>
                                                        <form action="{{ route('admin.users.destroy', $user->id) }}" method="POST" onsubmit="return confirm('Bạn có chắc muốn xóa?');">
                                                            ...
                                                            @method('DELETE')
                                                            <button type="submit" class="block px-4 py-1.5 text-base transition-all duration-200 ease-linear text-slate-600 dropdown-item hover:bg-slate-100 hover:text-slate-500 focus:bg-slate-100 focus:text-slate-500 dark:text-zink-100 dark:hover:bg-zink-500 dark:hover:text-zink-200 dark:focus:bg-zink-500 dark:focus:text-zink-200">
                                                                <i data-lucide="trash-2" class="inline-block size-3 ltr:mr-1 rtl:ml-1"></i> Xóa
                                                            </button>
                                                        </form>
